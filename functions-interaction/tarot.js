        ...
        request(
            ...
            async function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    ...
                    tarotcross(body,client,interaction);
                }
                else {
                    client.api.interactions(interaction.id, interaction.token).callback.post({
                        data: {
                            type: 4,
                            data: {
                                content: "There was an error fetching tarot cards. Try again later."
                            }
                        }
                    })
                    console.log(error);
        ...
        request(
            ...
            async function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    console.log(body);
                    console.log(body[0].name);
                    tarotthree(body,client,interaction);
                }
                else {
                    client.api.interactions(interaction.id, interaction.token).callback.post({
                        data: {
        ...
        request(
            ...
            async function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    console.log(body);
                    console.log(body[0].name);
                    var embedd = singleTarot(body[0])
                    client.api.interactions(interaction.id, interaction.token).callback.post({
                        data: {
                            type: 4,
                            data: await createAPIMessage(interaction,embedd,client)
                        }
                    })
                }
                else {
...
function singleTarot(card) {
    ...
    embed.setFooter('Tarot Single');
    return embed;
}

function tarotthree(cardarray, client, interaction) {
    ...
    Promise.all(promisearray).then(async (res) => {
        ...
        client.api.interactions(interaction.id, interaction.token).callback.post({
            ...
            client.channels.resolve(interaction.channel_id).send({ files: [{ 
                attachment: buffer,
...
function tarotcross(cardarray, client, interaction) {

...
export default function(msg,client) {
    var day = "today";
    if (msg.content.toLowerCase().search("tomorrow") != -1) {
        day = 'tomorrow'
    }
    else if (msg.content.toLowerCase().search("yesterday") != -1) {
        day = 'yesterday'
    }
    var sign = 'null';
    var url = 'null';
    signsgroup.forEach((signpair) => {
        if (msg.content.toLowerCase().search(signpair.sign) != -1) {
            sign = signpair.sign;
            url = signpair.url;
        }
    })
    if (sign != "null") {
        aztrojs.getAllHoroscope(sign, (res) => {
        ...
        msg.channel.send("Please include the name of the sign you want a horoscope for.")
    }
}
